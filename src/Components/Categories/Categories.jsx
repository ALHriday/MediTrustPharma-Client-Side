        ...
        <div className="py-6 px-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                <Link to='/shop' onClick={() => setProducts(allData)} className="text-black" >
                    <div className="relative rounded-md">
                        <div className="h-[80px] rounded-t-md rounded-r-md shadow-inner flex justify-center items-center -mb-2">
                            <img className="object-cover w-full h-full rounded-md opacity-70 z-10" src='https://media.istockphoto.com/id/1224142418/vector/doctors-prescribing-various-meds-for-patient-big-pills-capsules-blisters-glass-bottles-with.jpg?s=612x612&w=0&k=20&c=Nxfs0hecA03eNcAqUGoFxsdu6uB08uxkEOPNXWC7RfU=' alt="" />
                        </div>
                ...
                {data && data.map((d, i) => <Link to='/shop' onClick={() => handleCategorySearch(d.categoryName)} className="text-black" key={i}>
                    <div className="relative rounded-md">
                        <div className="h-[80px] rounded-t-md rounded-r-md shadow-inner flex justify-center items-center -mb-2">
                            <img className="object-cover w-full h-full rounded-md opacity-70 z-10" src={d.categoryImage} alt="" />
                        </div>
                        <div className="rounded-md">
                            <h1 className="font-bold text-sm sm:text-xl rounded-md bg-slate-50 opacity-85 shadow-inner p-2">{d.categoryName}</h1>
                        </div>
                    </div>
                </Link>
                )}
